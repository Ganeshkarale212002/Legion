...
async def cb_handler(client: Client, query: CallbackQuery):
        ...
        if chat_type == enums.ChatType.PRIVATE:
            ...
            if grpid is not None:
                ...
                try:
                    ...
                    return await query.answer('🔆彡[ FHDmovies24x7 ]彡🔆')
            else:
    ...
    if query.data.startswith("file"):
        ...
        await query.answer('🔆彡[ FHDmovies24x7 ]彡🔆')
...
async def auto_filter(client, msg, spoll=False):
    if not spoll:
        ...
        if 2 < len(message.text) < 100:
            search = message.text
            search_ = re.sub(
                r"\b(pl(i|e)*?(s|z+|ease|se|ese|(e+)s(e)?)|netflix|movie(s)?|dub(bed)*|new|-|:|series|file|subtitle(s)*|season(s)*|episode(s)*|anime|kdrama|horror|hd|full\smovie|with\ssubtitle(s)?)",
...
async def advantage_spell_chok(msg):
    query = re.sub(
        r"\b(pl(i|e)*?(s|z+|ease|se|ese|(e+)s(e)?)|netflix|movie(s)?|dub(bed)*|new|:|series|file|season(s)*|episode(s)*|anime|subtitle(s)*|kdrama|horror|hd|full\smovie||with\ssubtitle(s)?)",
